...
function createElementModule() {
  return gulp.src(`./${config.src.path}/${config.src.entrypoint}`)
    .pipe(modifyFile((content) => {
      content = content.replace(new RegExp(`../node_modules/${config.mixin.scope}/`, 'g'), '../../');
      content = content.replace(new RegExp('../node_modules/', 'g'), '../../../');
      return content;
    }))
    .pipe(rename({
...
function createElementScript() {
  return gulp.src(`./${config.src.path}/${config.src.entrypoint}`)
    .pipe(modifyFile((content) => {
      ...
      // Replace catalyst element's imports with globally accessible object imports.
...
gulp.task('build-module', gulp.series('create-element:module', () => {
  return gulp.src(`./${config.temp.path}/${config.mixin.name}.js`)
    .pipe(gulp.dest(`./${config.dist.path}`));
}));

// Build the es5 script version of the component.
gulp.task('build-script', gulp.series('create-element:script', () => {
  return gulp.src(`./${config.temp.path}/${config.mixin.name}.script.js`)
    .pipe(webpackStream({
      target: 'web',
      mode: 'production',
      output: {
